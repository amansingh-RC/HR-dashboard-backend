...
router.get('/', async function(req, res) {
  try {
    ...
    const latestResult = await pool.query('SELECT date FROM attendance ORDER BY date DESC LIMIT 1');
    const latestDate = latestResult.rows.length > 0 ? latestResult.rows[0].date : '';
    ...
    const presentMap = {};
    deptPresent.forEach(function(r) { presentMap[r.department] = r.present; });
    const absentMap = {};
    deptAbsent.forEach(function(r) { absentMap[r.department] = r.absent; });
